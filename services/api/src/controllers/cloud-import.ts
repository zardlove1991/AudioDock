...
export class CloudImportController {
  ...
  @Get('providers')
  @LogMethod()
  async getProviders(): Promise<{ code: number; message: string; data: CloudProvider[] }> {
  ...
  @Get('auth/:provider')
  @LogMethod()
  async getAuthUrl(@Param('provider') provider: string): Promise<{ code: number; message: string; data?: { url: string } }> {
  ...
  @Post('auth/callback')
  @LogMethod()
  async handleAuthCallback(@Body() callback: CloudAuthCallback): Promise<{ code: number; message: string; data?: any }> {
    try {
      const tokens = await this.cloudImportService.handleAuthCallback(callback.provider, callback.code, callback.state);
      return { code: 200, message: 'success', data: tokens };
    } catch (error) {
      return { code: 400, message: error instanceof Error ? error.message : 'Auth failed' };
    }
  }
  ...
  async listFiles(
    ...
    try {
      const files = await this.cloudImportService.listFiles(provider, path || '/', accessToken);
      return { code: 200, message: 'success', data: files };
    } catch (error) {
      return { code: 400, message: error instanceof Error ? error.message : 'Failed to list files' };
    }
  }
  ...
  @Post('import')
  @LogMethod()
  async createImportTask(@Body() request: CloudImportRequest): Promise<{ code: number; message: string; data?: { taskId: string } }> {
    try {
      const taskId = await this.cloudImportService.createImportTask(request);
      return { code: 200, message: 'success', data: { taskId } };
    } catch (error) {
      return { code: 400, message: error instanceof Error ? error.message : 'Import task creation failed' };
    }
  }
  ...
  @Get('task/:id')
  @LogMethod()
  async getImportTask(@Param('id') id: string): Promise<{ code: number; message: string; data?: any }> {
  ...
  async getDownloadUrl(
    ...
    try {
      const downloadUrl = await this.cloudImportService.getDownloadUrl(id, fileId);
      if (!downloadUrl) {
        return { code: 404, message: 'Download URL not found' };
      }
      return { code: 200, message: 'success', data: { url: downloadUrl } };
    } catch (error) {
      return { code: 400, message: error instanceof Error ? error.message : 'Failed to get download URL' };
    }
  }
}